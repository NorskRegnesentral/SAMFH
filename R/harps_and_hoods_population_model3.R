...
data$SSstart = fish$Year[1]												#State-space process starts from this year
...
data$xmax = nrow(fish)	#Length of state-space process
data$CQuota = c(0,0)											#Catch level in future projections
data$suitability = fish$suit              #Suitability index based on cod and capelin
...
minit = 0.5									#Location parameter
rinit = 0.5									#Rate parameter
Scinit = 1                  #Scaling parameter
...
parameters$logm = log(minit)
parameters$logr = log(rinit)
##parameters$logSc = log(Scinit)
##parameters$logSigma = log(sigmainit)
##parameters$u=rep(0,length.out = (data$Nc+data$Npred-data$SSstart+data$Cdata[1,1]))


obj <- MakeADFun(data,parameters,DLL="harps_and_hoods_population_model3")
