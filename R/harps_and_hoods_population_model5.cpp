...
//  PARAMETER(logitp);                       //Mixture proportion for observation error
  PARAMETER_VECTOR(Ftemp);                     //Random effect vector (unobserved pregnancy rate)
  ...
  Type beta = logBeta;  //Does not need to be positive?
  Type betaCap = ilogit(logBetaCap);
  Type betaCod = ilogit(logBetaCod);
