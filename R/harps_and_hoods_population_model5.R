...
compile("./R/harps_and_hoods_population_model5.cpp",
        "-O1 -g", DLLFLAGS="")
dyn.load(dynlib("./R/harps_and_hoods_population_model5"))
...
bCapinit = 0.4								#Slope parameter for capelin
bCodinit = 0.6								#Slope parameter for cod
...
parameters$logBeta = log(binit)    #Doesnt need to be positive?
parameters$logBetaCap = logit(bCapinit) # sign(bCapinit) * log(abs(bCapinit))
parameters$logBetaCod = logit(bCodinit) # sign(bCodinit) * log(abs(bCodinit))
parameters$logSdProc = log(Procinit)
parameters$logSdObs = log(Obsinit)
##parameters$logitp = log(Prinit)
parameters$Ftemp=rep(0,length.out = (data$Nc+data$Npred))


obj <- MakeADFun(data,parameters,DLL="harps_and_hoods_population_model5")
